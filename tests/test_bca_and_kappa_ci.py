...
def test_bca_ci_covers_true_mean_at_nominal_rate():
    """Loose coverage check: BCa should cover a known mean at ~95% over many trials."""
    rng = random.Random(1)
    true_p = 0.65
    covers = 0
    trials = 60
    ...
    # 60 trials, nominal 95%. Allow wide tolerance to avoid flakiness while
    # still failing if coverage collapses.
    assert coverage >= 0.80, f"BCa coverage {coverage} at n=100 was too low"
...
def test_bca_and_percentile_agree_on_symmetric_data():
    ...
    # On symmetric data, BCa and percentile should be within a fraction of the
    # scale of each other.
...
def test_kappa_bootstrap_ci_wider_at_small_n():
    """Kappa CI width should shrink as n grows."""
    rng = random.Random(3)
    ...
    assert width_small > width_large, (
        f"kappa CI did not shrink with n: small={width_small}, large={width_large}"
    )
