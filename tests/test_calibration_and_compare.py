...
def test_cohen_kappa_perfect_and_chance():
    assert abs(cohen_kappa(['A', 'B', 'A'], ['A', 'B', 'A']) - 1.0) < 1e-09
...
def test_calibration_report_shapes():
    j = HeuristicJudge()
    exs = [PreferenceExample('Explain load balancing.', 'A load balancer spreads traffic across servers.', 'It depends on many factors.', Preference.A), PreferenceExample('What is Kubernetes?', 'Kubernetes orchestrates containers across nodes.', 'Hard to say really.', Preference.A)]
...
def test_compare_policies_recovers_better_policy():
    ...
    rows = []
    for i in range(20):
        rows.append({'prompt': f'Explain topic {i} clearly.', 'policy_a': 'It depends and there are many factors to consider.', 'policy_b': f'Topic {i} works by doing X, with a concrete relevant example about topic {i}.'})
