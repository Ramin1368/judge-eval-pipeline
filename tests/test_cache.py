...
def test_llm_judge_uses_cache_on_hit():
    ...
    cache = InMemoryCache()
    j = DigitalOceanLLMJudge(api_key="", cache=cache)  # empty key forces fallback

    # First call: fallback runs, but nothing is written because no live call succeeded.
    v1 = j._decide("p", "a", "b")
    assert isinstance(v1, JudgeVerdict)
    assert cache.stats.writes == 0  # fallback verdicts are not cached

    # Prime the cache and confirm the judge returns cached values on hit.
