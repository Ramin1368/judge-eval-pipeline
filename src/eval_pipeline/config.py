...
Uses ``yaml`` if installed, otherwise falls back to a tiny hand-written
parser sufficient for the schema in ``config/config.yaml`` (flat key: value
and one level of nesting, no lists needed today). Kept dependency-free by
design so the pipeline runs with only the Python stdlib available.
"""
...
class PipelineConfig:
    ...
    inference_env: dict = field(default_factory=lambda: {
        "base_url_env": "DO_INFERENCE_BASE_URL",
        "api_key_env": "DO_INFERENCE_API_KEY",
