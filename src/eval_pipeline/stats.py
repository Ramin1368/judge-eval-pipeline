...
def percentile_bootstrap_ci(scores: Sequence[float], n_boot: int=10000, alpha: float=0.05, seed: int=12345, statistic: Callable[[Sequence[float]], float]=win_rate) -> tuple[float, float]:
    if not scores:
        return (0.0, 1.0)
...
def bca_bootstrap_ci(values: Sequence, statistic: Callable[[Sequence], float], n_boot: int=10000, alpha: float=0.05, seed: int=12345) -> tuple[float, float]:
    if not values:
        return (0.0, 1.0)
    n = len(values)
    rng = random.Random(seed)
...
def bootstrap_ci(scores: Sequence[float], n_boot: int=10000, alpha: float=0.05, seed: int=12345) -> tuple[float, float]:
...
def kappa_bootstrap_ci(labels_true: Sequence[str], labels_pred: Sequence[str], n_boot: int=2000, alpha: float=0.05, seed: int=12345) -> tuple[float, float]:
...
def summarize_scores(scores: Sequence[float], seed: int=12345) -> dict:
    ...
    return {'n': n, 'wins_a': wins_a, 'wins_b': wins_b, 'ties': ties, 'win_rate_b': win_rate(scores), 'bootstrap_ci': bca, 'bca_ci': bca, 'percentile_ci': pct, 'wilson_ci': wilson_ci(wins_b, wins_a + wins_b), 'p_value': sign_test_p(wins_b, wins_a), 'mde': minimum_detectable_effect(n)}
