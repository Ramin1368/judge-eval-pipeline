...
class Judge(ABC):
    ...
    def judge(self, prompt: str, response_a: str, response_b: str) -> JudgeVerdict:
        ...
        if forward.preferred == swapped_remapped:
            return JudgeVerdict(preferred=forward.preferred, confidence=(forward.confidence + swapped.confidence) / 2, rationale=forward.rationale, position_unstable=False)
        return JudgeVerdict(preferred=Preference.TIE, confidence=min(forward.confidence, swapped.confidence), rationale='verdict flipped on order swap, treated as tie', position_unstable=True)
