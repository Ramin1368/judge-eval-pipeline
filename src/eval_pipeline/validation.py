...
def validate_and_dedupe(examples: list[PreferenceExample]) -> tuple[list[PreferenceExample], ValidationReport]:
    ...
    for key, members in groups.items():
        ...
        if len(members) > 1:
            ...
            if len(set(canonical_labels)) > 1:
                contradictory += 1
                messages.append(f'contradictory labels for prompt={members[0].prompt[:40]!r}: {_fmt(counts)} resolved by majority')
        top = counts.most_common()
        if len(top) >= 2 and top[0][1] == top[1][1] and (Preference.TIE not in {top[0][0]}):
            quarantined += 1
            messages.append(f'unresolvable split {_fmt(counts)} for prompt={members[0].prompt[:40]!r} quarantined from calibration')
            continue
        ...
        if first.response_a <= first.response_b:
            ...
            resolved = {Preference.A: Preference.B, Preference.B: Preference.A, Preference.TIE: Preference.TIE}[resolved_canonical]
        clean.append(PreferenceExample(prompt=first.prompt, response_a=first.response_a, response_b=first.response_b, preferred=resolved, example_id=first.example_id, annotator_id=first.annotator_id))
    consistency = consistency_hits / consistency_total if consistency_total else None
    report = ValidationReport(n_input=len(examples), n_clean=len(clean), n_quarantined=quarantined, duplicate_groups=dup_groups, contradictory_groups=contradictory, order_swapped_groups=order_swapped, human_self_consistency=consistency, fleiss_kappa=fleiss_kappa(examples), messages=messages)
    return (clean, report)
