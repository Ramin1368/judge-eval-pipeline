...
def calibrate_judge(judge: Judge, examples: list[PreferenceExample], slice_fn=None) -> CalibrationReport:
    ...
    if position_bias > POSITION_BIAS_MAX:
        notes.append(
            f"position bias rate {position_bias:.3f} above {POSITION_BIAS_MAX}, judge is order sensitive, "
            f"results rely on both order averaging"
        )
    if tie_judge > 0.6:
    ...
    return CalibrationReport(
        ...
        tie_rate_human=tie_human,
        tie_rate_judge=tie_judge,
        per_slice=per_slice,
