...
def compare_policies(
    ...
    if length_controlled is not None and abs(length_controlled - wr) >= 0.08:
        notes.append(
            f"raw win rate {wr:.3f} vs length controlled {length_controlled:.3f}, "
            f"gap suggests the result is partly driven by response length, prefer the length controlled figure"
        )
