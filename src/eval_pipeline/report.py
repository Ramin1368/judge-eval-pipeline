...
def build_report(calibration: CalibrationReport, comparison: PolicyComparisonResult, validation: ValidationReport | None=None, judge_name: str='judge', judge_table: list[dict] | None=None, cache_stats: dict | None=None) -> str:
    c, m = (calibration, comparison)
    if c.trustworthy and (not c.trustworthy_with_reserve):
        trust = 'TRUSTWORTHY'
    elif c.trustworthy_with_reserve:
        trust = 'TRUSTWORTHY WITH RESERVE'
    else:
        trust = 'USE WITH CAUTION'
    if m.winner.startswith('inconclusive'):
        headline = f"**Result: inconclusive.** We cannot say policy '{m.policy_b}' and '{m.policy_a}' differ. The estimated win rate for {m.policy_b} is {_pct(m.win_rate_b)}, but the 95 percent confidence interval [{_pct(m.ci_low)}, {_pct(m.ci_high)}] includes 50 percent, so the difference is not statistically distinguishable from a coin flip."
    else:
        headline = f"**Result: '{m.winner}' is the better policy.** On the held out set, '{m.policy_b}' wins {_pct(m.win_rate_b)} of head to head comparisons against '{m.policy_a}' (95 percent CI [{_pct(m.ci_low)}, {_pct(m.ci_high)}], p = {m.p_value:.4f}). Because the whole interval sits on one side of 50 percent, the result is statistically meaningful, not noise."
    lines: list[str] = []
    ...
    lines.append(f"How much should you trust this? The automated judge was checked against real human preferences before we used it, and it rated **{trust}** (chance corrected agreement, Cohen's kappa, of {c.cohen_kappa:.2f}{kappa_ci}). " + ('Treat the verdict above as reliable.\n' if c.trustworthy and (not c.trustworthy_with_reserve) else 'Treat the verdict above as directional; the kappa lower bound sits under the trust gate, so the same judge on a different sample might not pass calibration.\n' if c.trustworthy else 'Treat the verdict above as directional, not definitive.\n'))
    if m.length_controlled_win_rate_b is not None:
        gap = abs(m.length_controlled_win_rate_b - m.win_rate_b)
        lines.append(f"Gaming check: when we restrict to prompts where both answers were similar in length, the win rate is {_pct(m.length_controlled_win_rate_b)} (vs {_pct(m.win_rate_b)} overall), so the result is {('not merely' if gap < 0.08 else 'partly')} a length effect.\n")
    ...
    if c.kappa_ci_low is not None:
        lines.append(f"Cohen's kappa: {c.cohen_kappa:.3f} (95% CI [{c.kappa_ci_low:.3f}, {c.kappa_ci_high:.3f}], trust gate at least 0.40; 'with reserve' means point clears the gate but CI does not)")
    else:
    ...
    if judge_table:
        ...
        for r in judge_table:
            ...
            lines.append(f"| {r['judge']} | {r['n']} | {_pct(r['accuracy'])} | {r['cohen_kappa']:.3f} | {_pct(r['position_bias_rate'])} | {ece} | {r['trustworthy']} |")
        lines.append('')
    ...
    if m.percentile_ci is not None:
        lines.append(f'Percentile bootstrap cross-check: [{_pct(m.percentile_ci[0])}, {_pct(m.percentile_ci[1])}]')
    lines.append(f'Significance: p = {m.p_value:.4f} ({m.significance_test})')
    if m.mde is not None:
        lines.append(f'Minimum detectable effect (alpha=0.05, power=0.80): {_pct(m.mde)} absolute shift from 0.5; results smaller than this cannot be reliably detected at this n')
    if m.length_controlled_win_rate_b is not None:
    ...
    if m.bt_win_prob_b is not None:
        ...
        lines.append(f'Bradley-Terry strengths: {m.policy_a}={m.bt_strength_a:.3f}, {m.policy_b}={m.bt_strength_b:.3f}, implied P(B preferred)={_pct(m.bt_win_prob_b)}{ci_txt}')
        lines.append('  Note: for two policies BT reduces to the win rate; the model earns its keep at 3+ policies and by providing a bootstrap CI on strengths for ranking stability.')
    for note in m.notes:
        lines.append(f'Note: {note}')
    lines.append('')
    if cache_stats is not None:
        lines.append('## Judge cache\n')
        lines.append(f"Hits: {cache_stats['hits']}, misses: {cache_stats['misses']}, writes: {cache_stats['writes']}, hit rate: {_pct(cache_stats['hit_rate'])}")
        lines.append('')
    if validation is not None:
        ...
        if v.fleiss_kappa is not None:
            lines.append(f'Inter annotator agreement (Fleiss kappa, variable rater count aware): {v.fleiss_kappa:.3f}')
...
def build_html_report(calibration: CalibrationReport, comparison: PolicyComparisonResult, validation: ValidationReport | None=None, judge_name: str='judge') -> str:
    m = comparison
    ...
    if calibration.trustworthy and (not calibration.trustworthy_with_reserve):
        trust = 'Trustworthy'
    elif calibration.trustworthy_with_reserve:
        trust = 'Trustworthy with reserve'
    else:
        trust = 'Use with caution'
...
def _ci_chart_svg(point: float, lo: float, hi: float) -> str:
    ...
    half = x(0.5)
    return f'<svg width="{w}" height="{h}" viewBox="0 0 {w} {h}"><line x1="{half}" y1="10" x2="{half}" y2="{h - 20}" stroke="#c33" stroke-dasharray="4 4"/><text x="{half}" y="{h - 4}" font-size="11" fill="#c33" text-anchor="middle">50%</text><line x1="{x(lo)}" y1="40" x2="{x(hi)}" y2="40" stroke="#0b3d63" stroke-width="3"/><line x1="{x(lo)}" y1="30" x2="{x(lo)}" y2="50" stroke="#0b3d63" stroke-width="3"/><line x1="{x(hi)}" y1="30" x2="{x(hi)}" y2="50" stroke="#0b3d63" stroke-width="3"/><circle cx="{x(point)}" cy="40" r="6" fill="#0b3d63"/><text x="{x(point)}" y="26" font-size="11" fill="#0b3d63" text-anchor="middle">{_pct(point)}</text></svg>'
...
def _reliability_svg(bins: list[dict]) -> str:
    ...
    parts = [f'<svg width="{w}" height="{h}" viewBox="0 0 {w} {h}">', f'<line x1="{x(0)}" y1="{y(0)}" x2="{x(1)}" y2="{y(1)}" stroke="#aab" stroke-dasharray="4 4"/>', f'<line x1="{pad}" y1="{pad}" x2="{pad}" y2="{h - pad}" stroke="#333"/>', f'<line x1="{pad}" y1="{h - pad}" x2="{w - pad}" y2="{h - pad}" stroke="#333"/>', f'<text x="{w / 2}" y="{h - 8}" font-size="11" text-anchor="middle">confidence</text>']
    pts = [(b['mean_confidence'], b['accuracy']) for b in bins if b['n'] > 0]
    for i in range(1, len(pts)):
        parts.append(f'<line x1="{x(pts[i - 1][0])}" y1="{y(pts[i - 1][1])}" x2="{x(pts[i][0])}" y2="{y(pts[i][1])}" stroke="#0b3d63" stroke-width="2"/>')
    for cx, cy in pts:
...
def _html_dq(v: ValidationReport | None) -> str:
    ...
    return f'{v.n_input} input rows collapsed to {v.n_clean} unique comparisons, {v.contradictory_groups} contradictions resolved by majority, {v.n_quarantined} quarantined{sc}{fk}.'
