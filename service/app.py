...
def _to_examples(items: list[LabeledItem]) -> list[PreferenceExample]:
    ...
    for it in items:
        out.append(PreferenceExample(
            prompt=it.prompt, response_a=it.response_a, response_b=it.response_b,
            preferred=Preference.parse(it.preferred), annotator_id=it.annotator_id,
        ))
    return out
...
def calibrate(req: CalibrateRequest) -> dict:
    ...
    return {
        ...
        "cohen_kappa": rep.cohen_kappa,
        "position_bias_rate": rep.position_bias_rate,
        "expected_calibration_error": rep.expected_calibration_error,
        "trustworthy": rep.trustworthy,
        "notes": rep.notes,
        "validation": {
            "n_input": validation.n_input,
            "n_clean": validation.n_clean,
            "n_quarantined": validation.n_quarantined,
            "human_self_consistency": validation.human_self_consistency,
            "fleiss_kappa": validation.fleiss_kappa,
        },
    }
...
def compare(req: CompareRequest) -> dict:
    ...
    return {
        ...
        "judge_trustworthy": calibration.trustworthy,
